<script setup lang="ts">
  ...
  const normalizedColumnWidths = computed(() => {
    ...
    const widths =
      props.widget.columnWidths?.length === props.widget.cols
  ...
  const normalizedRowHeights = computed(() => {
    ...
    const heights =
      props.widget.rowHeights?.length === props.widget.rows
  ...
  const tableMode = computed<'simple' | 'complex' | 'legacy'>(
    () => props.widget.tableMode ?? 'legacy'
  )
  ...
  const isEditingCell = (renderRowIndex: number, colIndex: number) => {
    return editingCell.value?.renderRow === renderRowIndex && editingCell.value?.col === colIndex
  }
  ...
  function ensureRowSelection(row: number) {
    const selection = editorStore.tableSelection
    const fullWidth =
      selection && selection.startCol === 0 && selection.endCol === props.widget.cols - 1
    const alreadySelected =
      fullWidth && selection && row >= selection.startRow && row <= selection.endRow
    if (alreadySelected) return
    editorStore.setTableSelection({
      startRow: row,
      endRow: row,
  ...
  function ensureColSelection(col: number) {
    const selection = editorStore.tableSelection
    const fullHeight =
      selection && selection.startRow === 0 && selection.endRow === props.widget.rows - 1
    const alreadySelected =
      fullHeight && selection && col >= selection.startCol && col <= selection.endCol
    if (alreadySelected) return
    editorStore.setTableSelection({
      startRow: 0,
  ...
  function buildPreviewRows() {
    ...
    const headerRows = props.widget.cells
      .slice(0, headerCount)
      .map(row => row.map(cell => cloneCell(cell)))
  ...
  const tableStyle = computed(() => {
    ...
    const getBorderStyle = (
      side: 'tableBorderTop' | 'tableBorderRight' | 'tableBorderBottom' | 'tableBorderLeft'
    ) => {
...
<template>
  <div
    ...
    :class="{
      ...
      'simple-table': isSimpleTable,
      'complex-table': isComplexTable
    }"
    @mouseup="onMouseUp"
  ...
  >
    ...
    ></div>
    <!-- 列头 (用于选择列) -->
    <div v-if="!isPreview && editorStore.selectedWidgetId === widget.id" class="col-headers">
      <div
    ...
    <div class="table-body-wrapper">
      <!-- 行头 (用于选择行) -->
      <div v-if="!isPreview && editorStore.selectedWidgetId === widget.id" class="row-headers">
        <div
          ...
          <tbody>
            ...
            >
              <template v-for="(cell, colIndex) in row" :key="colIndex">
                ...
                >
                  <span v-if="getCellBindingKey(rowIndex, colIndex, cell)" class="cell-binding-tag">
...
<style scoped>
  ...
  .table-container.complex-table td {
