      ...
      <div class="qr-pattern">
        ...
        <div class="qr-content">
          <div v-for="i in 9" :key="i" class="qr-row">
            <div v-for="j in 9" :key="j" class="qr-cell" :class="{ filled: Math.random() > 0.5 }"></div>
          </div>
        </div>
      </div>
      <div class="qr-value" :title="displayValue">{{ displayValue.substring(0, 20) }}{{ displayValue.length > 20 ? '...' : '' }}</div>
    </div>
  </div>
</template>
