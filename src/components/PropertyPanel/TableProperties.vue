<script setup lang="ts">
  ...
  const hasColumnBindings = computed(
    () => Object.keys(props.widget.columnBindings || {}).length > 0
  )
  ...
  const selectedColumnIndex = computed(() =>
    tableSelection.value ? tableSelection.value.startCol : null
  )
  ...
  function updateTableBorderField(
    key: 'tableBorderWidth' | 'tableBorderColor' | 'tableBorderStyle',
    value: any
  ) {
    updateWidgetFields({ [key]: value })
  }

  function updateCellBorderField(
    key: 'cellBorderWidth' | 'cellBorderColor' | 'cellBorderStyle',
    value: any
  ) {
    updateWidgetFields({ [key]: value })
  }
  ...
  function getEffectiveTableBorderStyle() {
    return props.widget.tableBorderStyle ?? props.widget.borderStyle ?? 'solid'
  }

  function getEffectiveCellBorderStyle() {
    return props.widget.cellBorderStyle ?? props.widget.borderStyle ?? 'solid'
  }

  const borderPresetValue = computed(() => {
    const hasTableSideOverrides = !!(
      props.widget.tableBorderTop ||
      props.widget.tableBorderRight ||
      props.widget.tableBorderBottom ||
      props.widget.tableBorderLeft
    )
    if (hasTableSideOverrides) return 'custom'

    const tableStyle = getEffectiveTableBorderStyle()
    const cellStyle = getEffectiveCellBorderStyle()

    if (tableStyle === 'solid' && cellStyle === 'dashed') return 'preset1'
    if (tableStyle === 'dashed' && cellStyle === 'dashed') return 'preset2'
  ...
  function applyBorderPreset(preset: 'preset1' | 'preset2' | 'preset3') {
    ...
    const cellStyle: 'solid' | 'dashed' = preset === 'preset3' ? 'solid' : 'dashed'

    const tableBorderWidth = props.widget.tableBorderWidth ?? props.widget.borderWidth ?? 1
    const tableBorderColor = props.widget.tableBorderColor ?? props.widget.borderColor ?? '#000000'
    const cellBorderWidth = props.widget.cellBorderWidth ?? props.widget.borderWidth ?? 1
    const cellBorderColor = props.widget.cellBorderColor ?? props.widget.borderColor ?? '#000000'
    ...
    updateWidgetFields({
      ...
      cellBorderWidth,
      cellBorderColor,
      cellBorderStyle: cellStyle,
      tableBorderTop: undefined,
      tableBorderRight: undefined,
      tableBorderBottom: undefined,
      tableBorderLeft: undefined
    })
  }
  ...
  function updateRowsCols(bodyRows: number, cols: number) {
    ...
    const oldCells = props.widget.cells
    const newCells = Array(totalRows)
      .fill(null)
      .map((_, rowIndex) =>
        Array(cols)
          .fill(null)
    ...
    const currentColumnFractions = normalizeFractions(
  ...
  function handleInsertRow(position: 'before' | 'after') {
    ...
    if (tableSelection.value) {
      targetRow =
        position === 'before' ? tableSelection.value.startRow : tableSelection.value.endRow
    } else {
  ...
  function handleInsertCol(position: 'before' | 'after') {
    ...
    if (tableSelection.value) {
      targetCol =
        position === 'before' ? tableSelection.value.startCol : tableSelection.value.endCol
    } else {
  ...
  function updateTableSideBorder(
  ...
  function getTableBorderWidth(side: TableBorderSide) {
    return (
      props.widget[side]?.width ?? props.widget.tableBorderWidth ?? props.widget.borderWidth ?? 1
    )
  }

  function getTableBorderColor(side: TableBorderSide) {
    return (
      props.widget[side]?.color ??
      props.widget.tableBorderColor ??
  ...
  const cellBorderTop = computed(() => ({
    ...
    width:
      activeCell.value?.borderTop?.width ??
      props.widget.cellBorderWidth ??
      props.widget.borderWidth ??
      1,
    color:
      activeCell.value?.borderTop?.color ??
      props.widget.cellBorderColor ??
      props.widget.borderColor ??
      '#000000'
  }))
  ...
  const cellBorderRight = computed(() => ({
    ...
    width:
      activeCell.value?.borderRight?.width ??
      props.widget.cellBorderWidth ??
      props.widget.borderWidth ??
      1,
    color:
      activeCell.value?.borderRight?.color ??
      props.widget.cellBorderColor ??
      props.widget.borderColor ??
      '#000000'
  }))
  ...
  const cellBorderBottom = computed(() => ({
    ...
    width:
      activeCell.value?.borderBottom?.width ??
      props.widget.cellBorderWidth ??
      props.widget.borderWidth ??
      1,
    color:
      activeCell.value?.borderBottom?.color ??
      props.widget.cellBorderColor ??
      props.widget.borderColor ??
      '#000000'
  }))
  ...
  const cellBorderLeft = computed(() => ({
    ...
    width:
      activeCell.value?.borderLeft?.width ??
      props.widget.cellBorderWidth ??
      props.widget.borderWidth ??
      1,
    color:
      activeCell.value?.borderLeft?.color ??
      props.widget.cellBorderColor ??
      props.widget.borderColor ??
      '#000000'
  }))
...
<template>
  ...
  <a-alert type="info" show-icon class="table-mode-alert">
    ...
    <template #description>
      ...
      <span v-else-if="isComplexMode"
        >复杂表格需在表头绑定列数据源，数据区会按照绑定结果自动填充。</span
      >
  ...
  <a-form :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }" size="small">
    ...
    <a-form-item label="外框(默认)">
      <a-space>
        ...
        <a-input
          ...
          @change="
            (e: Event) =>
              updateTableBorderField('tableBorderColor', (e.target as HTMLInputElement).value)
          "
        />
        <a-select
          :value="getEffectiveTableBorderStyle()"
          style="width: 70px"
    ...
    <a-form-item label="单元格(默认)">
      <a-space>
        ...
        <a-input
          ...
          @change="
            (e: Event) =>
              updateCellBorderField('cellBorderColor', (e.target as HTMLInputElement).value)
          "
        />
        <a-select
          :value="getEffectiveCellBorderStyle()"
          style="width: 70px"
    ...
    <a-form
      ...
      size="small"
    >
      <a-divider orientation="left" style="font-size: 11px; margin: 8px 0">外框(四边)</a-divider>
      <a-form-item label="上">
        <a-space>
          <a-input-number
            :value="getTableBorderWidth('tableBorderTop')"
            :min="0"
            :max="5"
            style="width: 50px"
            @change="(v: number) => updateTableSideBorder('tableBorderTop', { width: v })"
          />
          <a-input
            type="color"
            :value="getTableBorderColor('tableBorderTop')"
            style="width: 36px"
            @change="
              (e: Event) =>
                updateTableSideBorder('tableBorderTop', {
                  color: (e.target as HTMLInputElement).value
                })
            "
          />
          <a-select
            :value="getTableBorderStyle('tableBorderTop')"
            style="width: 65px"
            @change="
              (v: 'solid' | 'dashed' | 'dotted' | 'none') =>
                updateTableSideBorder('tableBorderTop', { style: v })
            "
          >
      ...
      <a-form-item label="右">
        <a-space>
          <a-input-number
            :value="getTableBorderWidth('tableBorderRight')"
            :min="0"
            :max="5"
            style="width: 50px"
            @change="(v: number) => updateTableSideBorder('tableBorderRight', { width: v })"
          />
          <a-input
            type="color"
            :value="getTableBorderColor('tableBorderRight')"
            style="width: 36px"
            @change="
              (e: Event) =>
                updateTableSideBorder('tableBorderRight', {
                  color: (e.target as HTMLInputElement).value
                })
            "
          />
          <a-select
            :value="getTableBorderStyle('tableBorderRight')"
            style="width: 65px"
            @change="
              (v: 'solid' | 'dashed' | 'dotted' | 'none') =>
                updateTableSideBorder('tableBorderRight', { style: v })
            "
          >
            <a-select-option value="solid">实线</a-select-option>
            <a-select-option value="dashed">虚线</a-select-option>
      ...
      <a-form-item label="下">
        <a-space>
          <a-input-number
            :value="getTableBorderWidth('tableBorderBottom')"
            :min="0"
            :max="5"
            style="width: 50px"
            @change="(v: number) => updateTableSideBorder('tableBorderBottom', { width: v })"
          />
          <a-input
            type="color"
            :value="getTableBorderColor('tableBorderBottom')"
            style="width: 36px"
            @change="
              (e: Event) =>
                updateTableSideBorder('tableBorderBottom', {
                  color: (e.target as HTMLInputElement).value
                })
            "
          />
          <a-select
            :value="getTableBorderStyle('tableBorderBottom')"
            style="width: 65px"
            @change="
              (v: 'solid' | 'dashed' | 'dotted' | 'none') =>
                updateTableSideBorder('tableBorderBottom', { style: v })
            "
          >
            <a-select-option value="solid">实线</a-select-option>
            <a-select-option value="dashed">虚线</a-select-option>
      ...
      <a-form-item label="左">
        <a-space>
          <a-input-number
            :value="getTableBorderWidth('tableBorderLeft')"
            :min="0"
            :max="5"
            style="width: 50px"
            @change="(v: number) => updateTableSideBorder('tableBorderLeft', { width: v })"
          />
          <a-input
            type="color"
            :value="getTableBorderColor('tableBorderLeft')"
            style="width: 36px"
            @change="
              (e: Event) =>
                updateTableSideBorder('tableBorderLeft', {
                  color: (e.target as HTMLInputElement).value
                })
            "
          />
          <a-select
            :value="getTableBorderStyle('tableBorderLeft')"
            style="width: 65px"
            @change="
              (v: 'solid' | 'dashed' | 'dotted' | 'none') =>
                updateTableSideBorder('tableBorderLeft', { style: v })
            "
          >
            <a-select-option value="solid">实线</a-select-option>
            <a-select-option value="dashed">虚线</a-select-option>
      ...
      <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }" size="small">
        <template v-if="tableSelection">
          <a-form-item label="上">
            <a-space>
              <a-input-number
                :value="cellBorderTop.width"
                :min="0"
                :max="5"
                style="width: 50px"
                @change="(v: number) => updateCellSideBorder('borderTop', { width: v })"
              />
              <a-input
                type="color"
                :value="cellBorderTop.color"
                style="width: 36px"
                @change="
                  (e: Event) =>
                    updateCellSideBorder('borderTop', {
                      color: (e.target as HTMLInputElement).value
                    })
                "
              />
              <a-select
                :value="cellBorderTop.style"
                style="width: 65px"
                @change="
                  (v: 'solid' | 'dashed' | 'dotted' | 'none') =>
                    updateCellSideBorder('borderTop', { style: v })
                "
              >
                <a-select-option value="solid">实线</a-select-option>
                <a-select-option value="dashed">虚线</a-select-option>
          ...
          <a-form-item label="右">
            <a-space>
              <a-input-number
                :value="cellBorderRight.width"
                :min="0"
                :max="5"
                style="width: 50px"
                @change="(v: number) => updateCellSideBorder('borderRight', { width: v })"
              />
              <a-input
                type="color"
                :value="cellBorderRight.color"
                style="width: 36px"
                @change="
                  (e: Event) =>
                    updateCellSideBorder('borderRight', {
                      color: (e.target as HTMLInputElement).value
                    })
                "
              />
              <a-select
                :value="cellBorderRight.style"
                style="width: 65px"
                @change="
                  (v: 'solid' | 'dashed' | 'dotted' | 'none') =>
                    updateCellSideBorder('borderRight', { style: v })
                "
              >
                <a-select-option value="solid">实线</a-select-option>
                <a-select-option value="dashed">虚线</a-select-option>
          ...
          <a-form-item label="下">
            <a-space>
              <a-input-number
                :value="cellBorderBottom.width"
                :min="0"
                :max="5"
                style="width: 50px"
                @change="(v: number) => updateCellSideBorder('borderBottom', { width: v })"
              />
              <a-input
                type="color"
                :value="cellBorderBottom.color"
                style="width: 36px"
                @change="
                  (e: Event) =>
                    updateCellSideBorder('borderBottom', {
                      color: (e.target as HTMLInputElement).value
                    })
                "
              />
              <a-select
                :value="cellBorderBottom.style"
                style="width: 65px"
                @change="
                  (v: 'solid' | 'dashed' | 'dotted' | 'none') =>
                    updateCellSideBorder('borderBottom', { style: v })
                "
              >
                <a-select-option value="solid">实线</a-select-option>
                <a-select-option value="dashed">虚线</a-select-option>
          ...
          <a-form-item label="左">
            <a-space>
              <a-input-number
                :value="cellBorderLeft.width"
                :min="0"
                :max="5"
                style="width: 50px"
                @change="(v: number) => updateCellSideBorder('borderLeft', { width: v })"
              />
              <a-input
                type="color"
                :value="cellBorderLeft.color"
                style="width: 36px"
                @change="
                  (e: Event) =>
                    updateCellSideBorder('borderLeft', {
                      color: (e.target as HTMLInputElement).value
                    })
                "
              />
              <a-select
                :value="cellBorderLeft.style"
                style="width: 65px"
                @change="
                  (v: 'solid' | 'dashed' | 'dotted' | 'none') =>
                    updateCellSideBorder('borderLeft', { style: v })
                "
              >
                <a-select-option value="solid">实线</a-select-option>
                <a-select-option value="dashed">虚线</a-select-option>
        ...
        <small v-else class="form-tip">请选择单元格后可设置选区边框</small>
      </a-form>
    </template>
  </a-form>
  ...
  <div class="table-ops">
    ...
    <a-space wrap size="small">
      <a-button size="small" :disabled="!canMerge" @click="handleMerge">合并单元格</a-button>
      <a-button size="small" :disabled="!canSplit" @click="handleSplit">拆分单元格</a-button>
      <a-button size="small" :disabled="isComplexMode" @click="handleInsertRow('before')"
        >上方插入行
      </a-button>
      <a-button size="small" :disabled="isComplexMode" @click="handleInsertRow('after')"
        >下方插入行
      </a-button>
      ...
      <a-button size="small" danger :disabled="!canDeleteRow" @click="handleDeleteRow"
        >删除选中行
      </a-button>
      <a-button size="small" danger :disabled="!canDeleteCol" @click="handleDeleteCol"
        >删除选中列
      </a-button>
    </a-space>
  </div>
  ...
  <div v-if="!isSimpleMode" class="table-cell-panel">
    <template v-if="isHeaderSelection && selectedColumnIndex !== null">
      <a-form :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }" size="small">
        <a-form-item label="目标列"> 第 {{ selectedColumnIndex + 1 }} 列</a-form-item>
        ...
        <a-form :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }" size="small">
          <a-form-item label="文本内容">
            <a-textarea
              ...
              @change="
                (e: Event) => handleCellContentChange((e.target as HTMLTextAreaElement).value)
              "
            />
          </a-form-item>
          <!-- 复杂表格不显示单元格级别的数据绑定 -->
